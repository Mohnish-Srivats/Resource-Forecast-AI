...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        response = await enhanced_agent.process_message(request)
        return response
...
async def get_chat_history(session_id: str):
    ...
    try:
        history = enhanced_agent.get_chat_history(session_id)
        if not history:
...
async def get_conversation_context(user_id: str, session_id: str):
    ...
    try:
        context = enhanced_agent.get_conversation_context(user_id, session_id)
        if not context:
    ...
    try:
        ...
        if format == "json":
            return {
                ...
                "agent_capabilities": enhanced_agent.get_agent_capabilities(),
                "conversation": [
                    {
